...
- (void)infoButtonClicked:(id)sender
{
    ...
    info = [[UIAlertView alloc] initWithTitle:@"iTorrent"
                                      message:@"The best way to download torrents!\nPlease contact me on Github with any issues or features you would like integrated."
                                     delegate:self
...
-(void) alertView:(UIAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex{
    ...
    if (alertView.tag == 1) {
        ...
        if (buttonIndex == 1) {
            ...
            [[UIApplication sharedApplication] openURL:[NSURL URLWithString: @"https://github.com/jtv7/iTorrent"]];
...
- (void)viewDidLoad {
    ...
    self.savedButton  = [[UIBarButtonItem alloc] initWithImage:[UIImage imageNamed:@"saved"] style:UIBarButtonItemStylePlain target:self action:@selector(savedItemsButtonClicked:)];
    
    self.navigationItem.rightBarButtonItem = self.savedButton;
...
- (void)savedItemsButtonClicked:(id)sender
{
...
- (void)activityCounterDidChange:(NSNotification*)notif
{
    ...
    if (num > 0) {
        ...
        [self.activityCounterBadge setBadgeString:[NSString stringWithFormat:@"%li", (long)num]];
