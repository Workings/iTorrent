...
printMessage(int level, const char * name, const char * message, const char * file, int line )
{
    ...
    if( name )
        DDLogVerbose(@"[%s] %s %s (%s:%d)", timestr, name, message, file, line );
}
...
- (void)resetToDefaultPreferences
{
    ...
    [fDefaults setObject:[self defaultDownloadDirLocal] forKey:@"DownloadFolder"];
    [fDefaults setObject:[self defaultDownloadDirLocal] forKey:@"IncompleteDownloadFolder"];
...
//	tr_variantDictAddBool(&settings, TR_KEY_START, [fDefaults boolForKey: @"AutoStartDownload"]);
	...
	tr_variantDictAddStr(&settings, TR_KEY_download_dir, [[self defaultDownloadDirLocal] cStringUsingEncoding:NSASCIIStringEncoding]);
...
//    [banner show];
    ...
    [RKDropdownAlert title:@"Error" message:err_msg backgroundColor:[UIColor colorWithRed:(173/255.0) green:(48/255.0) blue:(48/255.0) alpha:1.f] textColor:[UIColor whiteColor] time:2.5];
...
//    [banner show];
    ...
    [RKDropdownAlert title:@"Message" message:msg backgroundColor:[UIColor colorWithRed:(48/255.0) green:(110/255.0) blue:(173/255.0) alpha:1.f] textColor:[UIColor whiteColor] time:2.5];
}
...
//    [banner show];
    ...
    [RKDropdownAlert title:@"Message" message:msg backgroundColor:[UIColor colorWithRed:(77/255.0) green:(175/255.0) blue:(67/255.0) alpha:1.f] textColor:[UIColor whiteColor] time:2.5];
